...
async function main() {
    ...
    // Check if cars already exist to avoid duplicate slug errors
    const existingCars = await prisma.car.count();
    if (existingCars > 0) {
        console.log('⏭️  Skipping car seeding (', existingCars, 'cars already exist)');
    } else {
        // Create cars only if none exist
