...
import { api, type Car, type Agent } from '../api';
...
export default function Cars() {
    ...
    const togglePublish = async (id: string) => {
        await api.toggleCarPublish(id);
        loadData();
    };

    const toggleFeatured = async (id: string) => {
        await api.toggleCarFeatured(id);
        loadData();
    };
        ...
        <div>
            <div className="page-header flex justify-between items-center">
                <div>
                    ...
                    <p className="page-subtitle">Manage your car inventory</p>
                </div>
                                ...
                                <tr key={car.id}>
                                    <td>
                                        <div className="flex items-center gap-2">
                                            <img src={car.images[0]} alt={car.title} className="avatar" style={{ borderRadius: '0.5rem' }} />
                                            <div>
                                    ...
                                    <td>
                                        <span className={`badge ${car.status === 'SALE' ? 'badge-success' : 'badge-primary'}`}>
                                            {car.status}
                                        </span>
                                    </td>
                                    <td>
                                        <button className={`toggle ${car.isPublished ? 'active' : ''}`} onClick={() => togglePublish(car.id)} />
                                    </td>
                                    <td>
                                        <button className={`toggle ${car.isFeatured ? 'active' : ''}`} onClick={() => toggleFeatured(car.id)} />
                                    </td>
                                    <td>
                                        <div className="actions">
                                            <button className="action-btn" onClick={() => openModal(car)}>✏️</button>
                                            <button className="action-btn danger" onClick={() => handleDelete(car.id)}>🗑️</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
